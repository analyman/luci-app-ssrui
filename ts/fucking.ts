    ...
    fucking.ElementsAccessor.links_group.addEventListener(UContentChange, function() {
        ...
        server_list.map( x => {
            ...
            <div><table class="table table-light rounded mt-2 border">
                <tr><td>Server :</td>           <td>${x.server}       </td></tr>
                <tr><td>Server Port :</td>      <td>${x.server_port}  </td></tr>
                <tr><td>Local Address :</td>    <td>${x.local_address}</td></tr>
                <tr><td>Local Port :</td>       <td>${x.local_port}   </td></tr>
                <tr><td>Password :</td>         <td>${x.password}     </td></tr>
                <tr><td>Method :</td>           <td>${x.method}       </td></tr>
                <tr><td>Protocol :</td>         <td>${x.protocol}     </td></tr>
                <tr><td>Protocol Parameter:</td><td>${x.protoprama}   </td></tr>
                <tr><td>Obfs:</td>              <td>${x.obfs}         </td></tr>
                <tr><td>Obfs Parameter :</td>   <td>${x.obfsparam}    </td></tr>
            </table></div>`
...
message_bar.addEventListener("beginUpdate", function(earg: CustomEvent){
    if(earg.detail.args == null)
        message_bar.ShowWithDuration("<div>begin update</div>", 1000 * 1.5);
    else if(earg.detail.args.length == 1)
        message_bar.ShowWithDuration("<div>begin update</div>", earg.detail.args[0]);
    else if(earg.detail.args.length >= 2)
        message_bar.ShowWithEvent("<div>begin update</div>", earg.detail.args[0], earg.detail.args[1]);
    return;
});
...
fucking.ElementsAccessor.address_bar.addEventListener("keydown", function(earg: KeyboardEvent) {
    ...
    if(!address_bar.validate()) {
        message_bar.ShowWithDuration(
            '<div class="bg-danger rounded m-2 p-2">Please input correct url, like "http://example.com/example"</div>', 3.9 * 1000);
        return
    }
    ...
    address_bar.hide();
    message_bar.ShowWithDuration(
        '<div class="bg-success rounded m-2 p-2">Please subscribe new ssr subscription in order to keep it</div>', 3.9 * 1000);
});
...
fucking.ElementsAccessor.subs_update_button.addEventListener("click", () => {
    ...
    if((fucking.ElementsAccessor.subscriptions_group.lastChild as Element) == (active_elem.parentNode as Element)) {
        message_bar.ShowWithDuration('<div class="bg-warning">Give me a address !!!</div>', 2 * 1000, true);
        return;
    }
    ...
    try {
        ...
        message_bar.ShowWithDuration('<div class="bg-warning">don\'t try this, no result !!!</div>', 2 * 1000, true);
        return;
    }
    in_updating = true;
    let fi: Promise<boolean> = sss.Subscribe();
        ...
        let old_remarks = fucking.VarAccessor.server_index.map(x => x["remarks"]).filter(x => x != null);
        nn.map( x => {
            if(old_remarks.indexOf(x["remarks"]) != -1)
                return;
            fucking.VarAccessor.server_index.push(x);
        });
        document.dispatchEvent(new CustomEvent(UServerListChange, {detail: "update"}));
        in_updating = false;
        return;
    }, () => {
        message_bar.ShowWithDuration('<div class="bg-warning">Subscribe fail !!!</div>', 2 * 1000, true);
        in_updating = false;
        return;
    });
    return;
...
fucking.ElementsAccessor.subs_delete_button.addEventListener("click", function() {
    ...
    if(pos == len - 1) {
        message_bar.ShowWithDuration(`<div class="bg-warn rounded"Don't try to do this.</div>"`, 2.2 * 1000);
        return;
    }
