...
-- handle form-data to update uci configure file
local table = http.formvaluetable("user.ssrui")
for k, v in next, table do
    if type(k) == "string" then
        if string.find(k, "[%w_]*%.[%w_]") then
            for sec, opt in string.gfind(k, "([%w_]*)%.([%w_]*)") do
                m:set(sec, opt, v)
                break
            end
        else
...
m.title	= translate("Shadowsocksr Web UI")
m.description = translate("Configure Shadowsocksr Client, which support subscription and manual editing")

m:section(SimpleSection).template = "ssrui/ssrui_status"
m:section(SimpleSection).template = "ssrui/ssr_config_editor"

--[[
obfs,     server_port, obfsparam,     server,      password, method,    group
protocol, protoparam,  local_address, local_port , time_out, fast_open, remarks
--]]
local server_list_json_file = "/etc/ssrui/server_list.json"

function json_file_to_var(filename) --[
    local file_handler = io.open(filename, "r")
    if file_handler == nil then
        return nil
    end
    local ret_var = jsonc.parse(file_handler:read("*a"))
    return ret_var
end --]

function set_list_values(list_value, value, dispaly, table_list)
    local i = 0
    if type(table_list) ~= "table" then return 0 end
    for _, val in ipairs(table_list) do
        if type(val) == "table" and type(val[value]) == "string" and type(val[dispaly]) == "string" then 
            list_value:value(val[value], translate(val[dispaly]))
            i = i + 1
        end
    end
    return i
